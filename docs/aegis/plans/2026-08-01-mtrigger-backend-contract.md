...
**Goal:** 同步 MTrigger 新 envelope、固定名称模板和 `memory_writeback_template`，并将有效记忆原样保存到玩家补充信息。
...
**Compatibility Boundary:** `alter_affection` 本地名称、非固定名称模板、`maica_quality_status`、手动玩家信息的 `[player]` 前缀和 `memory_concl_arc` 保持不变。
...
- [x] 新增 `memory_writeback_template = MTriggerTemplate("memory_writeback_template", "memory_item", ...)`、规范模板注册、两个固定名称映射和 memory 上限。
...
- [x] 添加失败的源契约测试：校验函数支持 `prefix_player=False`；`write_memory` 使用该参数并只在返回非 None 时 append；注册 `MTriggerBase(memory_writeback_template, "write_memory", ...)`。
- [x] 运行 `python -m pytest tests/test_backend_v13_compat.py -q -k "memory_writeback_template or player_addition"`，预期失败。
