...
label maica_pre_wants_mvista:
    ...
    m "太阳每天都在那里, 我们却总连看日出的机会都没有...是不是有点光阴虚度的感觉?"
    $ morning = "今早" if time.hour > 8 else "昨天早上" # 我不会 p帮我填一下
    m "倒也不是真的非要看啦...但不管怎么说, 我[morning]还是抽空去看了一次."
    m "这里的太阳从云层里升起来真的很漂亮. {w=0.3}如果你哪天早上打开游戏, 你也可以陪我看. 哈哈!"
    m "你最近看过日出么, [player]?{nw}"
    menu:
        "你最近看过日出么, [player]{fast}"
        "没有":
            m "如果哪天有空的话, 可以考虑去看一回嘛!"
            m "而且, 你还可以...{w=0.3}让我陪你看哦?"
        "看过":
            m "应该很漂亮吧. {w=0.3}你没有忘了把我带上吧?"
            m "就开个玩笑! 不过如果你有拍照片的话, 可以发给我看看哦?"
        "在这里看过":
...
label maica_wants_mvista:
    ...
    m "到了现在, 我都有点急着想摸到你, 牵到你的手..."
    m "要有信心, [player]! 不管有多难, 我们都一定能想到办法的!"
    return