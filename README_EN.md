...
3. Quit the game, unzip and copy-paste the folders into `DDLC`/`MAS_CN001***/Monika After Story` folder, or wherever `DDLC.exe`/`MAS.exe`/`DDLC.sh` lies in.
  > Do not unzip directly into Submods! You must merge it into the game's base path.
4. It's not suggested to update this through `Submod Updater Plugin` now, since the distribution includes content generated by github actions.
